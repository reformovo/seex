# Seex 0.1.x Completed Work

> Archived from the active roadmap on 2026-07-31. Completed task and metric
> wording is preserved for historical review; current and future work lives in
...
Only completed items remain below. Outstanding native-query, RSS, Metal, and
release work moved to active ROADMAP U2 and U6.

###### Stage 1: Baseline and Budget Calibration

- [x] Add release/test-support resource counters for each panel/generation:
...
- [x] Derive Overview and Detail budgets from logical plot width `L`. Use
...
- [x] Keep budgets independent of Run/Metric count, visibility, and
...
- [x] Let each `CurveSeriesSnapshot` retain one chart `Series` plus series-level
...
- [x] Remove duplicate `AlignedMetricPoint`/`DataPoint` ownership from Viewer
  ...
  evidence.
- [x] Preserve Phase 2 aligned queries, the public Python API, native catalog,
  Parquet schema, storage reduction, and comparison semantics.

###### Stage 3: Bounded GPUI Rendering Memory

- [x] Preallocate render compaction from logical canvas bucket count rather than
  input point count, and evict projection/path entries for removed Runs and
  Metrics.
- [x] Reduce long-lived `Path<Pixels>` ownership and deep copies. Retain only
  ...
  CPU threshold.
- [x] Preserve visible tracks plus one viewport of overscan. Off-screen tracks
  must not prepare paths, and hover emphasis must not issue queries or replace
  immutable snapshots.

###### Stage 4: Lower Native Query Peaks

- [x] Keep four-way bounded reads and the 100 ms trailing debounce so multiple
  ...
  merged snapshot.
- [x] Do not reduce visible Metric count, selected Run count, or interaction
  refresh rate to obtain lower memory use.

###### Stage 5: Resource and Release Validation

- [x] Cover 1x/2x/3x display scale, sparse/dense windows, spikes, viewport
